...
const cardScript: CardScript = {
    // Elusive. (The first time this creature is attacked each turn, no damage is dealt.) Fight/Reap: You may play or use a Logos card this turn.
    power: () => 1,
    elusive: () => true,
    reap: {
        validTargets: (state) => {
            const creatures = friendlyCreatures(state).filter(x => (x as Creature).backingCard.house === House.Logos)
            const artifacts = friendlyArtifacts(state).filter(x => (x as Artifact).backingCard.house === House.Logos)
            const cards = activePlayerState(state).hand.filter(x => (x as CardInGame).backingCard.house === House.Logos)
...
cardScripts.scripts.set("logos-ambassador", cardScript)