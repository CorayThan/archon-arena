...
class GameScene extends Phaser.Scene {
    ...
    preload() {
        this.load.image(ImageEnum.CARDBACK, cardback)
        this.load.image(ImageEnum.UNFORGED_KEY, unforgedKey)
        this.load.image(ImageEnum.FORGED_KEY, forgedKey)
        this.load.image(ImageEnum.DAMAGE_TOKEN, damage)
        this.load.image(ImageEnum.AMBER_TOKEN, amber)
        this.load.image(ImageEnum.STUN_TOKEN, stun)
        this.load.image(ImageEnum.ARMOR_TOKEN, armor)
        this.load.image(ImageEnum.POWER_TOKEN, power)
        this.load.image(ImageEnum.CHAINS, chains)
        this.load.image(ImageEnum.DOOM_TOKEN, chains)
        this.load.image(ImageEnum.UNDER_CONSTRUCTION, underConstruction)
        this.load.image(ImageEnum.CARD_POWER, cardPower)
        this.load.image(ImageEnum.ENHANCED_CARD_POWER, enhancedCardPower)
        this.load.image(ImageEnum.GREEN_CARD_GLOW, greenCardGlow)
        this.load.image(ImageEnum.ORANGE_CARD_GLOW, orangeCardGlow)
    ...
    renderPlayerBoard(player: PlayerState, originX: number, originY: number, playerPosition: PlayerPosition) {
        ...
        const chainsImage = new Phaser.GameObjects.Image(this, originX + playerDataOffsetX + 5, originY + 85, ImageEnum.CHAINS)
        ...
        for (let i = 0; i < 3; i++) {
            const textureId = player.keys >= i + 1 ? ImageEnum.FORGED_KEY : ImageEnum.UNFORGED_KEY
            const keySize = 40
        ...
        const discardPileZone = this.createCardDropZone({
            ...
            getCardImage: () => player.discard.length === 0 ? ImageEnum.CARDBACK : player.discard[player.discard.length - 1].backingCard.cardTitle,
        ...
        const purgePileZone = this.createCardDropZone({
            ...
            x: purgePileX,
            y: originY + CARD_HEIGHT / 2,
            getCardImage: () => player.purged.length === 0 ? ImageEnum.CARDBACK : player.purged[player.purged.length - 1].backingCard.cardTitle,
        ...
        for (let i = 0; i < player.artifacts.length; i++) {
            ...
            if (!artifact.ready) {
                artifactOffsetX += SMALL_CARD_HEIGHT * 0.1
                artifactOffsetX += (SMALL_CARD_HEIGHT * 0.1) * (artifact.cardsUnderneath.length)
            }
        ...
        for (let i = 0; i < player.creatures.length; i++) {
            ...
            if (!creature.ready) {
                creatureOffsetX += SMALL_CARD_HEIGHT * 0.1
                creatureOffsetX += (SMALL_CARD_HEIGHT * 0.1) * (creature.cardsUnderneath.length)
            }

            const creatureCard = new SmallCard({
                scene: this,
                x: originX + SMALL_CARD_WIDTH / 2 + (SMALL_CARD_WIDTH + SMALL_CARD_WIDTH * 0.1) * (i + 1) + creatureOffsetX,
                y: creatureY,
                id: creature.id,
        ...
        for (let i = 0; i < player.hand.length; i++) {
            const card = new Card({
                ...
                y: originY + CARD_HEIGHT / 2,
                id: player.hand[i].id,
                front: playerPosition === PlayerPosition.TOP ? ImageEnum.CARDBACK : player.hand[i].backingCard.cardTitle,
                back: ImageEnum.CARDBACK,
    ...
    createCardDropZone(
        {
            ...
            y,
            onDrop,
            getCardImage = () => ImageEnum.CARDBACK,
            getMinimumAlpha = () => 0.1,
            visible = true,
